        ...
        for index in range(num2Send):
            ...
            if kk==0:
                ...
                if index>=77 and index<=85:
                    arr[index].phase += kk*(index-81)*P_CHANGE 
                elif index>=72 and index<=76:
                    arr[index].phase = kk*(72-index)*P_CHANGE
                elif index>=86 and index<=89:
                    arr[index].phase = kk*(90-index)*P_CHANGE
                elif index>=103 and index<=109:
                    arr[index].phase += kk*(index-106)*P_CHANGE
                elif index==102:
                    arr[index].phase += kk*2*P_CHANGE
                elif index==97:
                    arr[index].phase += kk*P_CHANGE
                elif index>=109 and index<=111:
                    arr[index].phase += kk*(112-index)*P_CHANGE
                elif index==115 or index==117:
                    arr[index].phase += kk*(1)*P_CHANGE
                elif index==114 or index==112:
                    arr[index].phase += kk*(-1)*P_CHANGE
                elif index==116:
                    arr[index].phase += kk*(2)*P_CHANGE
                elif index==113:
                ...
                #upper
                    ...
                    arr[index].phase += kk*(33-index)*P_CHANGE
                elif index>=24 and index<=28:
                    arr[index].phase += kk*(index-24)*P_CHANGE
                elif index>=38 and index<=41:
                    arr[index].phase += kk*(index-42)*P_CHANGE
                elif index>=51 and index<=57:
                    arr[index].phase += kk*(54-index)*P_CHANGE
                elif index>=48 and index<=50:
                    arr[index].phase += kk*(index-48)*P_CHANGE
                elif index>=58 and index<=59:
                    arr[index].phase += kk*(60-index)*P_CHANGE
                elif index==62 or index==60:
                    arr[index].phase += kk*(1)*P_CHANGE
                elif index==63 or index==65:
                    arr[index].phase += kk*(-1)*P_CHANGE
                elif index==61:
                    arr[index].phase += kk*(2)*P_CHANGE
                elif index==64:
